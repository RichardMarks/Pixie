...
//MG: 2007/04/14 Modified - Added tga_read_from_Buffer
...
static tga_result tga_read_rle_buffer(tga_image *dest, char *buffer);
 ...
 *          valid.
 */
tga_result tga_read_from_Buffer(tga_image *dest, char* buffer)
{
    #define BARF(errcode) \
        { tga_free_buffers(dest);  return errcode; }

    #define READ(destptr, size) \
		{ memcpy(destptr,buffer, size); buffer+=size; }

    #define READ16(dest) \
        { memcpy(&dest,buffer, 2); buffer+=2;		\
          dest = letoh16(dest); }
    ...
    {
        /* read RLE */
        tga_result result = tga_read_rle_buffer(dest, buffer);
        if (result != TGA_NOERR) BARF(result);
    }
    else
    {
